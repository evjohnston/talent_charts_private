...
year_breaks_latest <- function(years, by = NULL) {
  ...
  # 2024, 2014, 2004, 1994, ... .
  breaks <- seq(from = yr_max, to = yr_min, by = -by)
  sort(unique(breaks))
}

scale_x_years <- function(
    years,
    by = NULL,
    pad_right = 0.01,
    pad_left = 0.01,
...
annotate_milestones <- function(
  ...
  list(
    ...
    geom_label(
      ...
      vjust         = 0.5,
      family        = FONT_FAMILY,
      size          = MILESTONE_LABEL_SIZE / .pt,
      color         = "grey25",
      linewidth     = 0,
      label.padding = unit(PUB$spacing$milestone_label_padding_lines, "lines"),
      label.r       = unit(0, "pt"),
      lineheight    = 0.95,
      show.legend   = FALSE
    ),
  ...
  c(
    ...
    list(
      ...
      geom_label(data = d,
                 ...
                 label.padding = unit(PUB$spacing$label_box_padding_pt, "pt"),
