...
TABLE_EXPORT_DPI    <- TABLE_CSS_DPI
...
# Compatibility aliases used by the base gt theme. Values come from PUB.
TABLE_DATA_FONT_PT     <- PUB$type$body_pt
TABLE_HEADER_FONT_PT   <- PUB$type$body_pt
TABLE_TITLE_FONT_PT    <- PUB$type$title_pt
TABLE_SUBTITLE_FONT_PT <- PUB$type$subtitle_pt
TABLE_SOURCE_FONT_PT   <- PUB$type$source_pt
...
TABLE_WIDTH_PROFILES <- list(
  ...
  apply_table_profile <- function(gt_tbl, profile = "standard", stub_width_px = NULL) {
    ...
    gt_tbl %>%
      tab_options(
        table.font.size        = px(pt_to_px_profile(PUB$type$body_pt)),
        table.width            = px(p$width_px),
        container.width        = px(p$width_px),
        column_labels.font.size = px(pt_to_px_profile(PUB$type$body_pt)),
        column_labels.padding   = px(pt_to_px_profile(PUB$spacing$table_col_pad_pt)),
        data_row.padding        = px(pt_to_px_profile(PUB$spacing$table_row_pad_pt)),
        heading.title.font.size    = px(pt_to_px_profile(PUB$type$title_pt)),
        heading.subtitle.font.size = px(pt_to_px_profile(PUB$type$subtitle_pt)),
        heading.padding            = px(pt_to_px_profile(PUB$spacing$table_heading_pad_pt)),
        source_notes.font.size     = px(pt_to_px_profile(PUB$type$source_pt)),
        source_notes.padding       = px(pt_to_px_profile(PUB$spacing$table_source_pad_pt))
      ) %>%
