...
resolve_stub_width <- function(gt_tbl,
  ...
  # An explicit stub width is authoritative: when the caller passes
  # stub_width_px (or with_stub_width()), the stub is set to exactly that,
  # whether wider or narrower than the labels' natural width. Auto-sizing (no
  # explicit value) still uses the natural width. The final clamp below still
  # guards the ceiling so a too-wide stub can't push data columns below their
  # floor.
...
scale_columns_to_width <- function(gt_tbl,
  ...
  out <- tryCatch(
    if (!is.na(stub_col)) {
      gt_tbl %>% cols_width(rlang::new_formula(rlang::sym(stub_col), rlang::expr(px(!!stub_w))))
    } else {
      gt_tbl %>% cols_width(stub() ~ px(stub_w))
    },
    error = function(e) {
      warning("Could not set the stub width via cols_width() [", conditionMessage(e),
              "]; falling back to the profile CSS width. ",
              "Stub and data columns may disagree.",
              call. = FALSE)
      gt_tbl
    }
  )
  ...
  # Fixed-layout tables ignore gt's per-column <col> widths in headless Chrome.
  # Re-assert each data column's computed width as id-scoped !important CSS,
  # keyed to nth-child position (stub is child 1, data cols are 2..n+1).
  id_tbl <- gt_tbl[["_options"]]
  tbl_id <- tryCatch(
    id_tbl$value[[which(id_tbl$parameter == "table_id")]],
    error = function(e) NULL
  )
  if (length(tbl_id) && !is.null(tbl_id) && !is.na(tbl_id) && nzchar(tbl_id)) {
    css_lines <- vapply(seq_along(data_cols), function(i) {
      w   <- widths[[data_cols[i]]]
      pos <- i + 1L
      sprintf(
        paste0("#%s tbody td:nth-child(%d) { width:%dpx !important; ",
               "min-width:%dpx !important; max-width:%dpx !important; }"),
        tbl_id, pos, w, w, w
      )
    }, character(1))
    out <- out %>% opt_css(css = paste(css_lines, collapse = "\n"))
  }
