...
fixed_table_width <- function(gt_tbl, width_px = TABLE_CSS_WIDTH_PX) {
  gt_tbl %>%
    tab_options(
    ...
    opt_css(css = sprintf(
      paste0(
        ...
        ".gt_table * { box-sizing: border-box; }\n",
        ".gt_table td, .gt_table th { box-sizing: border-box !important; }\n",
        ...
        ".gt_table .gt_row { white-space: normal !important; overflow-wrap: normal !important; word-break: normal !important; }\n",
        ".gt_table .gt_col_heading { white-space: normal !important; overflow-wrap: normal !important; word-break: normal !important; }\n",
      ...
      TABLE_STUB_WIDTH_PX, TABLE_STUB_WIDTH_PX, TABLE_STUB_WIDTH_PX
    ))
}
...
# year-over-year relative change columns for the interior years
# (chg_cols, e.g. "Chg2019", "Chg2021"), and a Change column
# (relative change, first to last year). Year selection, recoding,
...
build_eb_combined_table <- function(meta,
                                    sources = c(`All countries` = "tab_003",
                                                China            = "tab_004",
                                                India            = "tab_005"),
...
build_peak_table <- function(name, meta, first, last,
  ...
  rows <- lapply(cats, function(c) {
    ...
    tibble(country = c,
      ...
      mutate(from_peak = (last_val - peak_val) / peak_val * 100)
  }) %>% bind_rows()
  ...
  rows %>%
    ...
    fmt_number(columns = from_peak, decimals = 0, force_sign = TRUE, pattern = "{x}%") %>%
    ...
    tab_source_note("Peak is each country's highest single-year certification total; \u201cfrom peak\u201d is the change to 2024.") %>%
    theme_gt_tpa(meta = meta) %>%
    with_size("standard")
}
...
build_perm_expiry_table <- function(meta, path = NULL,
  ...
  if (is.null(path)) path <- file.path(DATA_DIR, "tab_029.csv")
  ...
  wide <- raw %>%
    ...
    mutate(
      ...
      share_net   = share_last - share_first
    ) %>%
    arrange(Country) %>%
  ...
  wide %>%
    gt(rowname_col = "Country", id = "tab_029") %>%
    ...
    tab_source_note("\u201cExpired\u201d columns show certifications that lapsed before use, in count and as a share of that year's certifications; Net is the change in that share, in percentage points.") %>%
    theme_gt_tpa(meta = meta) %>%
    opt_css(css = sprintf(
      "#%s .gt_column_spanner { border-bottom: 2px solid %s !important; }",
      "tab_029", tpa_colors[1])) %>%
    with_size("standard")
}

# ---- Build Employment Section Intentions Bookend Table ----
build_sector_bookend_table <- function(meta,
                                       sources = c(`U.S. citizens`          = "fig_070",
                                                   `Temporary visa holders` = "fig_069"),
  ...
  pull_group <- function(name, key) {
    ...
    tibble(
      ...
      mutate(!!paste0(key, "_net") := .data[[paste0(key, "_last")]] -
  ...
  tbl %>%
    ...
    tab_style(style = cell_text(align = "left"), locations = cells_stub(rows = TRUE)) %>%
    tab_source_note("1994 and 2024 are shares of PhDs with definite commitments (%); Net is the change in percentage points.") %>%
    theme_gt_tpa(meta = meta) %>%
    opt_css(css = sprintf(
      "#%s .gt_column_spanner { border-bottom: 2px solid %s !important; }",
...
build_bookend_table <- function(name, meta,
  ...
  df <- tibble(
    ...
    mutate(
      ...
      Change   = if (change_type == "relative")
        (pct_last - pct_first) / pct_first * 100
      else
        pct_last - pct_first
    )
...
# <level>, Change. change_type controls change *formatting* only. NA cells
...
build_ranking_delta_table <- function(df, name, meta, level_cols, chg_cols,
                                      ...
                                      change_type         = c("absolute", "relative"),
                                      footnote            = NULL,
                                      cell_footnote       = NULL,
...
build_field_share_table <- function(df, name, meta, field_labels) {
  ...
  tbl %>%
    ...
    cols_width(
      Country              ~ px(table_scaled_px(150)),
...
build_conference_summary_table <- function(name, meta, conf_names,
  ...
  summ <- wide %>%
    ...
    ungroup() %>%
    mutate(
      US_Change  = US_Last - US_First,
      CN_Change  = CN_Last - CN_First,
      Coverage   = paste0(first_yr, "\u2013", last_yr),
      Conference = recode(conf, !!!conf_names),
      cross_yr   = as.character(cross_yr)
    ) %>%
    arrange(is.na(cross_yr), cross_yr, desc(CN_Last)) %>%
  ...
  summ %>%
    ...
    fmt_number(columns = c(US_Change, CN_Change),
               decimals = 1, force_sign = TRUE, pattern = "{x} pp") %>%
    ...
    tab_footnote(
      footnote = paste(
        ...
        "that window is shown under Coverage and differs by venue. Change is in",
        "percentage points. \u201CChina overtook\u201D is the first such year",
...
# bookend levels, interior year-over-year relative deltas, a net relative
# change), but levels are dollar amounts and rows are grouped by country
# (U.S. top, China bottom). df arrives pivoted wide with a Country group
# column, a Type stub, dollar levels in level_cols, relative-change
# interior columns in chg_cols, and a relative net-change column named
# Change. Ordering and grouping happen upstream.
build_rd_type_delta_table <- function(df, name, meta, level_cols, chg_cols,
  ...
  if (is.null(footnote)) {
    footnote <- paste(
      ...
      "(PPP converted). The interior columns show the relative change from the",
      "prior shown year rather than the level. Net Change is the relative change",
      "across the full window. Basic research is investment in future capability;",
...
build_patents_table <- function(name, meta, sources,
                                ...
                                change_type = c("relative", "absolute")) {
  ...
  per_field <- lapply(seq_along(sources), function(i) {
    ...
    tibble(
      ...
      mutate(Change = if (change_type == "relative")
        (Last - First) / First * 100 else Last - First) %>%
...
# count and share in each year, the absolute count change, and the
# percentage-point share change. Two spanners group the count block and the
# share block. Only the share-change column carries the diverging color scale
# by default -- the pp move is the analytic point, the count change is context;
...
build_count_share_bookend_table <- function(name, meta,
  ...
  df <- tibble(
    ...
    mutate(
      cnt_chg = cnt_last - cnt_first,   # absolute count change
      sh_chg  = sh_last  - sh_first     # percentage-point share change
    )
  ...
  tbl <- df %>%
    ...
    fmt_number(columns = sh_chg, decimals = 1, force_sign = TRUE, pattern = "{x} pp") %>%
...
# change_type controls change *formatting* only ("absolute" = signed
# counts, "relative" = signed percent); compute matching values upstream.
...
save_table <- function(gt_tbl, name, size = NULL) {
  if (is.null(size)) {
  ...
  gt_tbl <- fixed_table_width(gt_tbl)
  
  gtsave(gt_tbl, html_path)
  gtsave(
    gt_tbl,
    png_path,
    vwidth  = TABLE_CSS_WIDTH_PX,
    vheight = unname(TABLE_EXPORT_HEIGHTS_PX[[size]]),
    zoom    = TABLE_EXPORT_ZOOM,
    expand  = 0
  )
  
  # Make Word interpret the exported bitmap as exactly 7.5 inches wide,
  # regardless of whether webshot adds/removes a pixel at the edge.
  if (requireNamespace("magick", quietly = TRUE)) {
    img <- magick::image_read(png_path)
    img_info <- magick::image_info(img)
    word_dpi <- round(img_info$width / WORD_TABLE_WIDTH_IN)
    magick::image_write(
      img,
      path    = png_path,
